...
def looks_like_person_name(name: str) -> bool:
    ...
    if len(name) > 80: return False
    tokens = [t for t in re.split(r"[\s\-]+", name.strip()) if t]
    if len(tokens) < 2: return False
    if sum(1 for t in tokens if t[:1].isupper()) < 2: return False
    # role contamination?
    if re.search(ALLOWED_ROLE_WORDS, name, re.I) or re.search(SENIORITY, name, re.I):
        return False
    return True

async def gpt_clean_name(name: str, role: str, email: str, page_url: str) -> str:
    """
    Returns a human personal name 'First Last [Middle/Particles allowed]' or '' if not a personal name.
    """
    base = (
...
async def process_uni(
    ...
    async with sem:
        ...
        # Local ranked pool for this university (min-heap of fixed size)
        # Stores tuples: (score, tiebreaker, row_dict)
        ...
        for i, fut in enumerate(tqdm(asyncio.as_completed(tasks), total=len(tasks), unit="page", desc=f"{uni.get('name','uni')} pages", leave=False)):
            ...
            for c in contacts:
                ...
                # Name cleaning (only when suspicious) if enabled
                original_name = (c.get("name") or "").strip()
                if verify_names:
                    needs_clean = (not looks_like_person_name(original_name)) or (len(original_name) > 0 and (re.search(ALLOWED_ROLE_WORDS, original_name, re.I) or re.search(SENIORITY, original_name, re.I)))
                    if needs_clean:
                        clean = await gpt_clean_name(original_name, c.get("role",""), email, c.get("page_url", page_url))
                        if clean:
                            c["name"] = clean
                        else:
                            c["name"] = ""  # will be filtered by keep_contact
                ...
                # If emit_all, still output everything (diagnostics), but we also maintain the pool for 'ok' rows
                if emit_all and not ok:
                    results.append(row)
        ...
        # Flush top-K pool for this uni (if using cap)
        if per_uni_max and per_uni_max > 0 and uni_pool:
            topk_sorted = sorted(uni_pool, key=lambda t: t[0], reverse=True)
            results.extend(r for (_, _, r) in topk_sorted)

# ───────── MAIN & ENTRYPOINT ─────────
async def main(country: str, limit: Optional[int], outfile: str, emit_all: bool, debug: bool, debug_dir: Optional[str], ignore_robots: bool, verbose: bool, browser_ua: bool, per_uni_max: Optional[int], verify_names: bool):
    ...
    try:
        ...
        tasks = [process_uni(u, country, sem, results, seen, stats, emit_all, per_uni_max, verify_names) for u in unis]
...
async def run_all(country: str, limit: Optional[int], outfile: str, emit_all: bool, debug: bool, debug_dir: Optional[str], ignore_robots: bool, verbose: bool, browser_ua: bool, per_uni_max: Optional[int], verify_names: bool):
    await main(country, limit, outfile, emit_all, debug, debug_dir, ignore_robots, verbose, browser_ua, per_uni_max, verify_names)
...
if __name__ == "__main__":
    ...
    p.add_argument("--per-uni-max", type=int, default=12, help="max contacts to keep per university (ranked by Score); set 0 to disable")
    p.add_argument("--verify-names", action="store_true", help="use an extra GPT pass to ensure Name is a real person (costs more tokens)")
